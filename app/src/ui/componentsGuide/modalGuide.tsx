...
export const modalGuide: IGuide = {
  ...
  options: [
      ...
      render: () => (
        <div>
          ...
          <ModalProvider value={new ModalRegister()}>
            ...
            <ModalConsumer>
              {modalRegister =>
                modalRegister.getModals().map(x => (
                  <PrivateModal key={`modal-${x.id}`} id={x.id}>
                    {x.children}
                  </PrivateModal>
                ))
              }
    ...
    },
    {
      ...
      render: () => (
        <div>
          <ModalLink open modalId="modal1" styling={"PrimaryButton"}>
            Open Modal
          </ModalLink>
          <ModalProvider value={new ModalRegister()}>
            <Modal id="modal1">
              <Section title={"Modal demo"}>
                <SimpleString>This is how to use a modal.</SimpleString>
                <ModalLink open={false} modalId="modal1" styling={"PrimaryButton"}>
                  Close Modal
                </ModalLink>
              </Section>
            </Modal>
