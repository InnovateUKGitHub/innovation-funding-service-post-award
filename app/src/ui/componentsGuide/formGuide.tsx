  ...
  options: [
    ...
    <ExampleForm.Form data={this.state.editor} onSubmit={() => this.onSave()} onChange={(dto) => this.onChange(dto)}>
        <ExampleForm.Fieldset heading="Example form for the editor">
            ...
            <ExampleForm.Checkboxes label="Checkbox option" name="option" hint="The option value" options={this.multiOptions} value={dto => dto.multipleOptions} update={(dto, options) => dto.multipleOptions = options}/>
...
interface ISimpleEditorDto {
  ...
  multipleOptions: { value: React.ReactNode; id: string }[] | null;
...
class SimpleForm extends React.Component<EmptyObject, { original: ISimpleEditorDto; editor: ISimpleEditorDto }> {
        ...
        <ExampleForm.Form data={this.state.editor} onSubmit={() => this.onSave()} onChange={dto => this.onChange(dto)}>
          <ExampleForm.Fieldset heading={`Example form for the editor ${this.state.original.name} `}>
            <ExampleForm.String
              ...
              update={(dto, value) => (dto.name = value)}
            />
            <ExampleForm.MultilineString
              label="Description"
              name="description"
              hint="A multiline field"
              value={data => data.description}
              update={(dto, value) => (dto.description = value)}
            />
            <ExampleForm.Numeric
              label="Value"
              name="value"
              hint="A numeric value"
              value={data => data.value}
              update={(dto, value) => (dto.value = value)}
            />
            <ExampleForm.Date
              label="Full date"
              name="name"
              hint="Full date input"
              value={data => data.date}
              update={(dto, date) => (dto.date = date)}
            />
            <ExampleForm.MonthYear
              label="Month year"
              name="name"
              hint="Month & year input"
              startOrEnd="start"
              value={data => data.date}
              update={(dto, date) => (dto.date = date)}
            />
            <ExampleForm.Radio
              label="Radio option"
              name="option"
              hint="The option value"
              inline
              options={this.options}
              value={dto => dto.option}
              update={(dto, option) => (dto.option = option)}
            />
            <ExampleForm.DropdownList
              label="Drop down list option"
              name="option"
              hint="The option value"
              hasEmptyOption
              options={this.dropdownOptions}
              value={dto => dto.dropdownOption}
              update={(dto, option) => (dto.option = option)}
            />
            <ExampleForm.Checkboxes
              label="Checkbox option"
              name="option"
              hint="The option value"
              options={this.multiOptions}
              value={dto => dto.multipleOptions}
              update={(dto, options) => (dto.multipleOptions = options)}
            />
