...
export const createTypedTable = <T,>() => {
  ...
  const TableComponent = ({
    ...
    const cols = children.map((column, columnIndex) => (
      <TableDataContextProvider value={{ mode: "col", columnIndex }}>{column}</TableDataContextProvider>
    ));
  ...
  const FullDateColumn = (props: ExternalColumnProps<DateConvertible>) => (
    <TableColumn {...props} renderCell={(data, index) => <FullDate value={props.value(data, index)} />} />
  );

  const FullNumericDateColumn = (props: ExternalColumnProps<DateConvertible>) => (
    <TableColumn {...props} renderCell={(data, index) => <FullNumericDate value={props.value(data, index)} />} />
  );

  const ShortDateColumn = (props: ExternalColumnProps<DateConvertible>) => (
    <TableColumn {...props} renderCell={(data, index) => <ShortDate value={props.value(data, index)} />} />
  );

  const ShortDateTimeColumn = (props: ExternalColumnProps<DateConvertible>) => (
    <TableColumn {...props} renderCell={(data, index) => <ShortDateTime value={props.value(data, index)} />} />
  );
