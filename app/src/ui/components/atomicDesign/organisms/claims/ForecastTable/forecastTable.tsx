...
import { NumberInput } from "../../../../bjss/inputs/numberInput";
import { AccessibilityText } from "../../../atoms/AccessibilityText/AccessibilityText";
...
export const ForecastTable = (props: Props) => {
  const { data, hideValidation, isSubmitting, editor } = props;

  useResizeTable();

  const periodId = getPeriodId(data.project, data.claims, data.claim);
  const intervals = calculateClaimPeriods(data);
  const tableRows = parseClaimData(data, editor, periodId, data.project.numberOfPeriods);
  ...
  function updateItem(
    ...
    if (cell) {
      const { partner, project, costCategories } = props.data;
    ...
    <div className="acc-sticky-table-container">
      <div className="acc-sticky-table-scrollable">
        <Table.Table
          data={tableRows}
          qa="forecast-table"
          headers={renderTableHeaders(periods, periodId, data.claim, data.IARDueOnClaimPeriods ?? [])}
          footers={renderTableFooters(
            periods,
            periodId,
            tableRows,
            ...
            <Table.Custom
              ...
              value={(x, index) =>
                renderForecastCell(
                  ...
                  isSubmitting || false,
                  updateItem,
                  props.allowRetroactiveForecastEdit ?? false,
                )
              }
              cellClassName={() =>
...
/**
 * finds the periodId from project and draftClaim.
 *
 * defaults to zero
 */
function getPeriodId(
  ...
  if (draftClaim) return Math.min(project.periodId, draftClaim.periodId) as PeriodId;
  ...
  if (approvedClaims.length === 0) return 0 as PeriodId;
  // If there are previously approved claims then return most recent
  return approvedClaims[0].periodId;
}

/**
 * calculates claim periods
 */
function calculateClaimPeriods(data: Pick<ForecastDataForTableLayout, "claimDetails" | "forecastDetails">) {
  const periods: { [k: string]: React.ReactElement<typeof CondensedDateRange> } = {};
  data.claimDetails.forEach(
    x => (periods[x.periodId] = <CondensedDateRange start={x.periodStart} end={x.periodEnd} />),
  );
  data.forecastDetails.forEach(
    x => (periods[x.periodId] = <CondensedDateRange start={x.periodStart} end={x.periodEnd} />),
  );
  return periods;
}
...
function parseClaimData(
  ...
  costCategories.forEach(category => {
    ...
    let currentPeriodId = 1 as PeriodId;

    while (currentPeriodId <= periodId) {
      addClaimDetailInfoToRow(currentPeriodId, category, data.claimDetails, row);
      currentPeriodId++;
    }

    if (numberOfPeriods) {
      while (currentPeriodId <= numberOfPeriods) {
        addForecastInfoToRow(currentPeriodId, category, forecasts, row);
        currentPeriodId++;
      }
    }
...
/**
 * adds submitted claim info to the row
 */
function addClaimDetailInfoToRow(
  periodId: PeriodId,
...
/**
 * adds forecast information to the row
 */
function addForecastInfoToRow(
  periodId: PeriodId,
  costCategory: Pick<CostCategoryDto, "id">,
...
/**
 * Component to render the table headers
 */
...
/**
 * renders the table footers
 */
function renderTableFooters(
  periods: string[],
  claimPeriod: number,
  parsed: TableRow[],
  ...
  cells.push(totals.map((value, index) => renderTableFooterCell(value, index, claimPeriod)));
  cells.push(
    renderTableFooterCell(
      costTotal,
      totals.length + 1,
      claimPeriod,
...
/**
 * renders individual forecast cells
 */
  ...
  return (
    <span>
      <NumberInput
        ...
        onChange={val => updateItem(editor.data, forecastRow.categoryId, periodId, dto => (dto.value = val as number))}
...
/**
 * renders individual table footer cell
 */
function renderTableFooterCell(total: number, key: number, claimedPeriod: number, className = "", qa?: string) {
  return (
    <td
      key={key}
      className={cx(
...
/**
 * provides animation to maintain table sticky columns
 */
function animate() {
...
const useResizeTable = () => {
  const debounce = useRef<null | ReturnType<typeof setTimeout>>(null);

  const requestAnimate = useCallback(() => {
    if (debounce.current) {
      clearTimeout(debounce.current);
    }
    debounce.current = setTimeout(() => requestAnimationFrame(animate), debounceTimeout);
  }, []);

  useEffect(() => {
    window.addEventListener("click", requestAnimate);
    window.addEventListener("resize", requestAnimate);
    window.addEventListener("keyup", requestAnimate);
    requestAnimate();

    return () => {
      window.removeEventListener("click", requestAnimate);
      window.removeEventListener("resize", requestAnimate);
      window.removeEventListener("keyup", requestAnimate);
    };
  }, [requestAnimate]);
};
