  ...
  return (
    <ACC.Section title={x => x.pcrSpendProfileDeleteCostContent.costSectionTitle(costCategory.name)}>
      ...
      <ACC.SummaryList qa="deleteOtherCost">
        <ACC.SummaryListItem label={x => x.pcrSpendProfileDeleteCostContent.labels.otherCosts.description} content={data.description} qa="description" />
        <ACC.SummaryListItem label={x => x.pcrSpendProfileDeleteCostContent.labels.otherCosts.totalCost} content={<ACC.Renderers.Currency value={data.value} />} qa="value" />
      </ACC.SummaryList>
    </ACC.Section>
  );
};
