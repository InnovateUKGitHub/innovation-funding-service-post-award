...
class Component extends React.Component<PcrStepProps<PCRItemForPartnerAdditionDto, PCRPartnerAdditionItemDtoValidator> & InnerProps> {
  ...
  private renderTemplateLinks(itemType: PCRItemTypeDto) {
    ...
    return (
      <ACC.Section>
        <ACC.LinksList
          openNewWindow
          links={itemType.files.map(x => ({ url: x.relativeUrl, text: (content: Content) => content.pcrAddPartnerStateAidEligibilityContent.labels.deMinimisDeclarationForm }))}
        />
      </ACC.Section>
    );
  }

  private renderDeMinimisForm(documentsEditor: IEditorStore<MultipleDocumentUploadDto, MultipleDocumentUploadDtoValidator>): React.ReactNode {
  ...
  private renderDeMinimisFiles(documentsEditor: IEditorStore<MultipleDocumentUploadDto, MultipleDocumentUploadDtoValidator>, documents: DocumentSummaryDto[]) {
    if (documents.length) {
      return (
        <ACC.Section
