...
> {
      ...
      <StoresConsumer>
        {stores => {
          return this.renderContents(
            queryString => stores.companies.getCompanies(queryString),
            this.state.queryString ? stores.companies.getCompanies(this.state.queryString, 10) : Pending.done([]),
          );
        }}
      </StoresConsumer>
    );
  ...
  renderContents(
    companiesHouseLoader: (x: string) => Pending<CompanyDto[]>,
    companiesHouseResults: Pending<CompanyDto[]>,
  ) {
      ...
      <ACC.Section qa="company-house" titleContent={x => x.pcrAddPartnerCompanyHouse.sectionTitle}>
        ...
        >
          ...
          <Form.Fieldset headingContent={x => x.pcrAddPartnerCompanyHouse.searchHeading} qa="search-companies-house">
            ...
            <Form.Button
              styling="Primary"
              name="companiesHouseSearch"
              onClick={() => this.setState({ queryString: this.state.searchTerm })}
            >
              <ACC.Content value={x => x.pcrAddPartnerCompanyHouse.searchButton} />
            </Form.Button>
          </Form.Fieldset>
          {this.renderPendingResults(Form, companiesHouseResults, this.props.pcrItem)}
          <Form.Fieldset qa="save-and-continue">
            <Form.Submit>
              <ACC.Content value={x => x.pcrAddPartnerCompanyHouse.pcrItem.submitButton} />
            </Form.Submit>
            <Form.Button name="saveAndReturnToSummary" onClick={() => this.props.onSave(true)}>
              <ACC.Content value={x => x.pcrAddPartnerCompanyHouse.pcrItem.returnToSummaryButton} />
            </Form.Button>
          </Form.Fieldset>
        </Form.Form>
      </ACC.Section>
  ...
  private onCompanySelectUpdate(
    value: ACC.SelectOption | null,
    ...
    return (
      <>
        ...
        <SimpleString className={"govuk-!-margin-bottom-0"}>
          {company.address.addressLine1}, {company.address.locality}, {company.address.postalCode},{" "}
