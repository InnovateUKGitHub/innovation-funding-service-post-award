...
const ClaimDetailDocumentsPage = (props: ClaimDetailDocumentsPageParams & BaseProps) => {
  ...
  return (
    <Page
      ...
      heading={getContent(x => x.pages.claimLineItems.documentsTitle({ costCategoryName: costCategory.name }))}
    >
      {/* Update the HTML title to include the costCategoryName*/}
      <Helmet>
        <title>
          {getContent(x => x.pages.claimLineItems.documentsHtmlTitle({ costCategoryName: costCategory.name }))}
        </title>
      </Helmet>
