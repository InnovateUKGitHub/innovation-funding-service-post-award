...
export const useViewForecastData = (projectId: ProjectId, partnerId: PartnerId) => {
  ...
  const project = mapToProjectDto(projectNode, ["id", "isActive", "status", "roles", "partnerRoles", "periodId"]);
  ...
  const claims = mapToClaimDtoArray(claimsGql, ["isApproved", "isFinalClaim", "paidDate"], {});
