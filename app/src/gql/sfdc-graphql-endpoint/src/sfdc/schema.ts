import { fieldNameNormalise } from '../utils/nameNormaliser.js';
import {
            ...
            return {
                ...
                get referencedEntities() {
                    return sObjectField.referenceTo.map(
                        (entityName) =>
                            schema.entities[fieldNameNormalise(entityName, { pascalCase: true })],
                    );
                },
            };
        } else {
            return {
                ...
                get referencedEntity() {
                    return schema.entities[
                        fieldNameNormalise(sObjectField.referenceTo[0], { pascalCase: true })
                    ];
                },
            };
        }
...
function createChildRelationShip(
    ...
    return {
        salesforceName: relationshipName,
        normalisedName: fieldNameNormalise(relationshipName),
        field,
        get entity() {
            return schema.entities[fieldNameNormalise(childSObject, { pascalCase: true })];
        },
    };
}
