...
export const configureFormRouter = ({
  ...
  singleFileFormHandlers.forEach(x => {
    result.post(getRoute(x), upload.single("attachment"), csrfProtection, handlePost({ schema })(x), handleError({ schema }));
  });

  multiFileFormHandlers.forEach(x => {
    result.post(getRoute(x), upload.array("attachment"), csrfProtection, handlePost({ schema })(x), handleError({ schema }));
  });
