...
export interface IPartnersApi {
  ...
  updatePartner: (params: ApiParams<{ partnerId: string, partnerDto: PartnerDto }>) => Promise<PartnerDto>;
}
...
class Controller extends ControllerBase<PartnerDto> implements IPartnersApi {
  ...
  constructor() {
    ...
    this.putItem("/:partnerId", (p, q, b) => ({ partnerId: p.partnerId, partnerDto: processDto(b) }), (p) => this.updatePartner(p));
  }
  ...
  public async updatePartner(params: ApiParams<{ partnerId: string, partnerDto: PartnerDto }>) {
    const context = contextProvider.start(params);
    await context.runCommand(new UpdatePartnerCommand(params.partnerDto));
