...
describe("GetClaimDocumentQuery", () => {
  it("should return result if document exists", async () => {
    ...
    const document = context.testData.createDocument(claim.Id, "cat", "jpg", "", "Meow Inc", "file content");

    const query = new GetClaimDocumentQuery(
      { projectId: project.Id, partnerId: partner.id, periodId: claim.Acc_ProjectPeriodNumber__c },
      document.Id,
    );
  ...
  it("should return null if document belongs to another project", async () => {
    ...
    const query = new GetClaimDocumentQuery(
      { projectId: project2.Id, partnerId: partner2.id, periodId: claim2.Acc_ProjectPeriodNumber__c },
      document.Id,
    );
    const result = await context.runQuery(query);
  ...
  it("should return null if document belongs to another partner", async () => {
    ...
    const query = new GetClaimDocumentQuery(
      { projectId: project.Id, partnerId: partner2.id, periodId: claim2.Acc_ProjectPeriodNumber__c },
      document.Id,
    );
    const result = await context.runQuery(query);
  ...
  it("should return null if document belongs to another claim", async () => {
    ...
    const query = new GetClaimDocumentQuery(
      { projectId: project.Id, partnerId: partner.id, periodId: claim2.Acc_ProjectPeriodNumber__c },
      document.Id,
    );
    const result = await context.runQuery(query);
