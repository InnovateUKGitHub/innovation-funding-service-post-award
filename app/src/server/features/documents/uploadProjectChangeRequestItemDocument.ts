...
export class UploadProjectChangeRequestItemDocumentCommand extends CommandBase<string[]> {
  constructor(private readonly projectId: string, private readonly projectChangeRequestItemId: string, private readonly documents: MultipleDocumentUploadDto) {
    super();
  }
  ...
  protected async Run(context: IContext) {
    ...
    for (const document of this.documents.files.filter(x => x.fileName && x.size)) {
      const id = await context.repositories.documents.insertDocument(document, this.projectChangeRequestItemId);
      results.push(id);
    }
