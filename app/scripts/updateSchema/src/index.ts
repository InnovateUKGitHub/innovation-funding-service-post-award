...
import { FilterObjectFields, FilterTypes, schemaFromExecutor } from "@graphql-tools/wrap";
...
const main = async () => {
  ...
  const transformedSchema = {
    ...
    transforms: [
      new FilterTypes(type => whitelist.includes(type.name)),
      new FilterObjectFields((typeName, fieldName) => {
        // filter out surplus delete operations that are generated by matcher RecordTypeDelete
        if (/Delete$/.test(fieldName)) {
          return /Acc_/.test(fieldName);
        }
        return true;
      }),
    ],
