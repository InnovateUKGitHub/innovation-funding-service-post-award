...
describe("UploadDocumentCommand", () => {
  const docToUpload = {content: "Hello world", fileName: "hello.txt", recordId: "TESTid"};
  it("should store a document and contentVersions", async () => {
    const context = new TestContext();

    const command = new UploadDocumentCommand(docToUpload.content, docToUpload.fileName, docToUpload.recordId);
  ...
  it("should have the correct content in contentDocumentLinks", async () => {
    ...
    expect(document.LinkedEntityId).toEqual(docToUpload.recordId);
  });
