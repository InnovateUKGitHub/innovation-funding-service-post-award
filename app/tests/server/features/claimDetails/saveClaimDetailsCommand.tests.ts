...
describe("SaveClaimDetails", () => {
  ...
  describe("Overheads calulation", () => {
    ...
    test("If multiple overhead categories will use one related to partner comp type and org type", async () => {
      ...
      const names = ["Labour", "Overheads", "Labour", "Overheads"];
      const costCategories = names.map((name) => context.testData.createCostCategory(x => x.Acc_CostCategoryName__c = name));

      // make first 2 one comp / org and second 2 a different comp / org
      [costCategories[0], costCategories[1]].forEach(x => {
        x.Acc_CompetitionType__c = "COMP_1";
        x.Acc_OrganisationType__c = "ORG_1";
      });

      [costCategories[2], costCategories[3]].forEach(x => {
        x.Acc_CompetitionType__c = "COMP_2";
        x.Acc_OrganisationType__c = "ORG_2";
      });
      ...
      const periodId = 2;
      const project = context.testData.createProject(x => x.Acc_CompetitionType__c = labourToUse.Acc_CompetitionType__c);
      const partner = context.testData.createPartner(project, x => x.Acc_OrganisationType__c = labourToUse.Acc_OrganisationType__c);
