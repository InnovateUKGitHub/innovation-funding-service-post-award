...
const makeKtpProject = (): CreateProjectProps => {
  const competition = competitionBuilder
    .create()
    .set({ Acc_CompetitionCode__c: "0", Acc_CompetitionName__c: "Competition", Acc_CompetitionType__c: "KTP" });
  ...
  const assAccount = defaultAccount.copy().set({ OrgMigrationId__c: "1", Name: "Hedge's Consulting Ltd." });
  const kbAdminAccount = defaultAccount.copy().set({ OrgMigrationId__c: "1", Name: "Hedge's Consulting Ltd." });
  const mainCompanyAccount = defaultAccount.copy().set({ OrgMigrationId__c: "1", Name: "Hedge's Consulting Ltd." });
  ...
  const [
    ...
    [assContact, assUser, assPcl],
    [kbAdminContact, kbUser, kbPcl],
    [mainCompanyContact, mainUser, mainPcl],
  ] = (<const>[
    ...
    ["4", assAccount, "associate@x.gov.uk", "Anna", "Sociate", "as", "Associate"],
    ["5", kbAdminAccount, "kbadmin@x.gov.uk", "Kristoff", "Baseman", "kb", "KB Admin"],
    ["6", mainCompanyAccount, "maincontact@x.gov.uk", "Mary", "Cabrera", "mc", "Main Company Contact"],
  ...
  const profileDetails = [
    accProfileDetailBuilder.create().set({
      ...
      Acc_CostCategoryDescription__c: "Associate Employment",
  ...
  return {
    ...
    logins: [
      ...
      { pcl: assPcl, user: assUser, contact: assContact, account: assAccount },
      { pcl: kbPcl, user: kbUser, contact: kbAdminContact, account: kbAdminAccount },
      { pcl: mainPcl, user: mainUser, contact: mainCompanyContact, account: mainCompanyAccount },
    ],
    pcrs: {
      headers: [],
      removePartner: [],
