...
export const useOnUpdateAddPartnerJesStep = () => {
  ...
  return useOnUpdate<z.output<JesStepSchema>, boolean, { link: ILinkInfo }>({
    req: data =>
      clientsideApiClient.pcrs.addPartnerJesStep({
        projectId,
        pcrId,
        pcrItemId: itemId,
        pcr: {
    ...
    onSuccess: async function (_: z.output<JesStepSchema>, __: boolean, context: { link: ILinkInfo } | undefined) {
      clearMessages();
      setFetchKey(k => k + 1);
