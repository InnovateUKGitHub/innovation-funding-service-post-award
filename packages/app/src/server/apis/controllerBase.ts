...
import { getErrorStatus, getErrorResponse } from "@framework/util/errorHandlers";
...
export interface RequestUrlParams extends UrlParamsBase, INominalTypes {}

export interface RequestQueryParams extends UrlParamsBase, INominalTypes {}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export type GetParams<T> = (params: RequestUrlParams, query: RequestQueryParams, body?: any) => T;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export abstract class ControllerBaseWithSummary<Context extends "client" | "server", TSummaryDto, TDto> {
  ...
  protected constructor(public path: string) {
    ...
    this.logger = new Logger(`${path} API controller`);
  }
  ...
  protected putItem<TParams>(
    path: string,
    getParams: GetParams<TParams>,
    run: Run<Context, TParams, TDto | null | boolean>,
  ) {
  ...
  protected postItem<TParams>(
    path: string,
    getParams: GetParams<TParams>,
    run: Run<Context, TParams, TDto | null | { id: string }>,
  ) {
    return this.postCustom<TParams, TDto | null | { id: string }>(path, null, getParams, run);
  }
