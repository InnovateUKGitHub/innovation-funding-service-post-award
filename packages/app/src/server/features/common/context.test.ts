import { SalesforceInvalidFilterError, FileTypeNotAllowedError } from "@server/repositories/errors";
...
describe("constructErrorResponse", () => {
  ...
  test.each`
    name                                                       | error
    ${"AppError if it is a SalesforceTokenError"}              | ${new TsforceTokenException({ message: "stub-token-error" })}
