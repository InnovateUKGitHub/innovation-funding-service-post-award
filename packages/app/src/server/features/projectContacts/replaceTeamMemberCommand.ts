...
import { pclIdValidation } from "@ui/zod/helperValidators/helperValidators.zod";
import { z } from "zod";
import { renamePartnerErrorMap } from "@ui/pages/pcrs/renamePartner/renamePartner.zod";

export type ReplaceTeamMemberPclDto = Pick<ProjectContactDto, "id">;

const schema = z.array(z.object({ id: pclIdValidation }));

export class ReplaceTeamMemberCommand extends ZodAuthorisedAsyncCommandBase<
  boolean,
  typeof schema,
  ReplaceTeamMemberPclDto
> {
  public runnableName = "UpdateProjectContactLinkCommand";

  constructor(
    private readonly projectId: ProjectId,
    private readonly contacts: ReplaceTeamMemberPclDto[],
  ) {
    super();
  }
  ...
  protected async getZodSchema() {
    return { schema, errorMap: renamePartnerErrorMap };
  }
  ...
  protected async runRepositoryCommands(context: IContext, validatedData: z.output<typeof schema>) {
